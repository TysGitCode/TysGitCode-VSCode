...
df.to_csv('trainingCategorized.csv', index=False)
...
# Split the data
X_train, X_test, y_train, y_test = train_test_split(df['TransactionDescription'], df['Category'], test_size=0.3)
...
# Define a new function for categorization
def smart_categorize_transaction(description):
    if categorize_transaction(description) == "Others":
        vec_description = vectorizer.transform([description])
        prediction_prob = clf.predict_proba(vec_description)

        # If the maximum prediction probability is below a threshold, say 0.7, label as "Review"
        if prediction_prob.max() < 0.85:
            return "Review"
        
        return clf.predict(vec_description)[0]
    else:
        return categorize_transaction(description)
        
new_df = pd.read_csv('allStatements.csv')
new_df['SmartCategory'] = new_df['TransactionDescription'].apply(smart_categorize_transaction)
new_df.to_csv('modelCategorizedAll.csv', index=False)

# Use the new function for categorization
new_df['Type'] = new_df['Amount'].apply(transaction_type)  # <-- Add this line
new_df['SmartCategory'] = new_df['TransactionDescription'].apply(smart_categorize_transaction)
new_df.to_csv('modelCategorizedAll.csv', index=False)
