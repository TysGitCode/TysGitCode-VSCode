...
# Load CSV
df = pd.read_csv('trainingCategorized.csv')
...
# Preparing the data
X_train = train_df['TransactionDescription']
y_train = train_df['Category']

X_test = test_df['TransactionDescription']
y_test = test_df['Category']

# Building a pipeline that first creates TF-IDF vectors and then runs a linear SVM
...
# If category not in predefined categories, predict using the model
def categorize_transaction(description):
    ...
    return category_found if category_found else predict_category(description)
...
# 1. Apply the machine's predictions to the entire dataset
df['MachinePredictedCategory'] = model.predict(df['TransactionDescription'])

# 2. Override the machine's predictions where a predefined keyword is found
def categorize_transaction(row):
    description = row['TransactionDescription']
    machine_prediction = row['MachinePredictedCategory']
    ...
    return category_found if category_found else machine_prediction

df['Category'] = df.apply(categorize_transaction, axis=1)

# Test the accuracy on the test set
print("Model Accuracy:", (model.predict(X_test) == y_test).mean())

# Applying categories to your data
df['Category'] = df.apply(categorize_transaction, axis=1)

df['Type'] = df['Amount'].apply(transaction_type)

df.to_csv('machineCategorized.csv', index=False)
